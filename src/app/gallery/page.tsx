            ...
            <section className="w-full min-h-screen text-black px-[150px] pt-25 space-y-6 bg-white pb-6">
                {galleryGroups.map((group, groupIndex) => (
                    <div
                        ...
                        className="w-full h-fit grid grid-cols-2 grid-rows-3 gap-2 [&>*]:bg-cover [&>*]:bg-center [&>*]:rounded mb-2"
                    >
                        {group.items.map((item, index) => {
                            if (item._type === 'doubleImage' && item.images) {
                                return (
                                    ...
                                    >
                                        {item.images.map((img, i) => (
                                            <div
                                                key={i}
                                                className="w-1/2 h-[236px] bg-cover bg-center bg-no-repeat rounded"
                                                style={{ backgroundImage: `url(${img.imageUrl})` }}
                                                role="img"
                                                aria-label={img.alt || `Gallery image ${groupIndex + 1}-${index + 1}-${i + 1}`}
                                            ></div>
                                        ))}
                                    </div>
                                );
                                ...
                                return (
                                    <div
                                        key={index}
                                        className="h-[236px] bg-cover bg-center bg-no-repeat"
                                        style={{ backgroundImage: `url(${item.imageUrl})` }}
                                        role="img"
                                        aria-label={item.alt || `Gallery image ${groupIndex + 1}-${index + 1}`}
                                    ></div>
                                );
                            }
                            return null;
