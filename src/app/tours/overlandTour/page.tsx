...
interface Tour {
  ...
  laQuotaComprende: string[];
  laQuotaNonComprende: string[];
}

export default async function OverlandMongoliaPage() {
  const tour: Tour | null = await getOverlandMongolia();
  ...
  return (
    <div className="bg-white">
      ...
      <section className="w-full min-h-screen text-black sm:px-37.5 px-[5%] pt-25 space-y-6 bg-white pb-6 relative">
        ...
        <div>
          {tour.laQuotaComprende.map((item, index) => (
        ...
        <div>
          {tour.laQuotaNonComprende.map((item, index) => (
            <div key={index} className="flex gap-4 mb-1">
              <p>✓</p>
              <p>{item}</p>
            </div>
...
//     description:
//       "Dopo colazione, partenza verso il Valle del Yol e sistemazione in campo turistico. Dopo pranzo ingresso nell’area del parco nazionale dei monti Gurvan Saikhan (Tre bellezze). Escursione nella valle di Yoliin Am, la “valle delle aquile”, habitat di numerose specie selvatiche. Fino a stagione avanzata si può passeggiare nel canyon al fresco di un'incredibile nevaio. Possibilità di fare l'escursione a cavallo. Pranzo e cena e pernottamento in campo turistico. BLD",
//   },
//   {
//     day: 17,
//     title: "TSAGAAN SUVRAGA. 280 km",
//     description:
//       "Partenza verso Tsagaan Suvraga. Lungo il percorso il paesaggio è caratterizzato da una profonda fenditura del terreno da est verso ovest. Quivi si ritrovano molti fossili di origine marina, in quanto la zona era un tempo sommersa dal mare. I colori delle rocce calcaree sono determinati da minerali di rame e di molibdeno. Nella zona vi sono numerosi dipinti rupestri, petroglifi e antiche iscrizioni.Pranzo, cena e pernottamento in campo turistico. BLD",
//   },
//   {
//     day: 18,
//     title: "BAGA GAZRIIN CHULUU. 240 km",
//     description:
//       "Partenza verso il deserto del Gobi (Dundgobi). Arrivo a Baga Gazriin Chuluu, suggestiva formazione rocciosa dove hanno vissuto in passato due monaci molto venerati, perciò meta di pellegrinaggi da parte della gente del posto. Lungo la strada si possono avvistare branchi di gazzelle, gruppi di cammelli e rari accampamenti di nomadi. Firmata per il pranzo a Mandalgobi il capoluogo di Dundgobi. Cena e pernottamento in campo turistico. BLD",
//   },
//   {
//     day: 19,
...
//     title: "PARTENZA",
//     description:
//       "Al mattino presto, trasferimento all’aereoporto e partenza l’Italia.",
//   },
// ];
...
//     id: 1,
//     description:
//       "Sistemazione: 2 notti in Hotel*** a Ulaanbaatar e 17 notti in Campi turistici nelle Gher durante il viaggio, sistemazione in camera doppia (singola a richiesta con supplemento) con bagni privati in hotel e con bagni condivisi nelle gher",
//   },
//   {
//     id: 2,
