...
export default async function TrekkingTourPage() {
  const tour = await getTrekkingTour();
  ...
  return (
    <div className="bg-white">
      ...
      <section className="w-full min-h-screen text-black px-[5%] pt-25 space-y-6 pb-6">
        ...
        <p className="font-semibold">Durazione: {tour.duration}</p>
        <p>{tour.dateRange}</p>
        ...
        <div className="flex flex-col gap-4">
          {tour.quataIndividuale.map((desc: string, i: number) => (
            <p key={i}>{desc}</p>
          ))}
        </div>
        ...
        {/* Lists */}
        {[
          ["La quota comprende", tour.laQuotaComprende],
          ["La quota non comprende", tour.laQuotaNonComprende],
          ["Suggerimenti per il viaggio", tour.travelSuggestions],
          ["I nostri vantaggi", tour.ourAdvantages],
