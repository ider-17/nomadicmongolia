...
export default async function CentroMongoliaPage() {
  const data: CentroMongoliaPageData | null = await getCentroMongoliaPage();
  ...
  return (
    <div className="bg-white h-screen">
      ...
      <section className="w-full min-h-screen text-black sm:px-37.5 px-[5%] pt-25 space-y-6 bg-white pb-6 relative">
        ...
        <div>
          {data.laQuotaComprende.map((text, index) => (
        ...
        <div>
          {data.laQuotaNonComprende.map((text, index) => (
            <div key={index} className="flex gap-4 mb-1">
              <p>✓</p>
              <p>{text}</p>
            </div>
